...
void grid_subsampling(vector<PointXYZ>& original_points,
	...
	for (auto& v : data) {
		subsampled_points.push_back(v.second.point * (1.0 / v.second.count));  // mean coordinate
		if (use_feature) {
		    float count = (float)v.second.count;
		    transform(v.second.features.begin(),
                      v.second.features.end(),
                      v.second.features.begin(),
                      [count](float f) { return f / count;});  // mean feature vector
		...
		if (use_classes) {
		    for (unsigned int i = 0; i < ldim; i++) {
		        subsampled_classes.push_back(max_element(  // take the label which appears most times
                    v.second.labels[i].begin(), v.second.labels[i].end(),
