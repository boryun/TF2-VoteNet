...
PyMODINIT_FUNC PyInit_grid_subsampling(void) {
    import_array();  // numpy C-API requested
...
static PyObject *grid_subsampling_compute(PyObject *self, PyObject *args, PyObject *keywds) {
    ...
    //! "method" argument is currently not handling.

	// Parse the input, //https://docs.python.org/zh-cn/3/c-api/arg.html#arg-parsing
	if (!PyArg_ParseTupleAndKeywords(
            args, keywds, "O|$OOfsi", kwlist, 
            &points_obj, &features_obj, &classes_obj, &sampleDl, &method_buffer, &verbose
        )
    ) {
    ...
    delete point_dims, delete feature_dims, delete classes_dims;
